...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(700, 700, "INF01047 - 00287695 - Rafael Baldasso Audibert", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //                |              +--- Tamamho do array de indices "indices[]" (veja função buildTriangles()).
        //                |              |                    +--- Os índices são do tipo "GLubyte" (8 bits sem sinal)
        //                |              |                    |                +--- Vértices começam em indices[0] (veja função buildTriangles()).
        //                |              |                    |                |
        //                V              V                    V                V
        glDrawElements(GL_TRIANGLE_FAN, POINTS_QUANTITY + 2, GL_UNSIGNED_BYTE, 0);
...
GLuint buildTriangles()
{
    ...
    //
    // Este vetor "NDC_coefficients" define a GEOMETRIA
    //
    GLfloat NDC_coefficients[(POINTS_QUANTITY + 1) * 4] = {0.0f};
    int i;
    double theta;

    NDC_coefficients[3] = 1.0f;
    for(i = 4, theta = 0; i <= POINTS_QUANTITY * 4; i += 4, theta += 2 * PI / POINTS_QUANTITY){
        NDC_coefficients[i + 0] = (float)(RADIUS * cos(theta));
        NDC_coefficients[i + 1] = (float)(RADIUS * sin(theta));
    ...
    GLfloat color_coefficients[(POINTS_QUANTITY + 1) * 4] = {0.0f}; // Inicializa todos com 0.0f por padrão
    color_coefficients[0] = 1.0f;
    color_coefficients[3] = 1.0f;
    for(i = 4; i <= POINTS_QUANTITY * 4; i += 4){
    ...
    //
    GLubyte indices[POINTS_QUANTITY + 2]; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
    for(int i=0; i <= POINTS_QUANTITY; i++){
        indices[i] = i;
    }
    indices[POINTS_QUANTITY + 1] = 1;
